/**
 * Backfill admission_academic_year + hybrid progression (set-based SQL).
...
async function main() {
  ...
  await client.query(
    `INSERT INTO institute_settings (key, value, updated_at)
     ...
     ON CONFLICT (key) DO UPDATE SET
       value = jsonb_set(
         COALESCE(institute_settings.value, '{}'::jsonb),
  ...
  // 1) Fill admission year from reg pattern 171XX##serial when missing
  ...
  `)
  console.log("Guessed admission from reg:", r1.rowCount)

  // 2) Mark still-missing
  ...
  `)
  console.log("Still missing admission (flagged):", r2.rowCount)

  // 3) Progress unlocked students that have admission year
  // computed = entry_study_year + (activeStart - admissionStart)
  ...
  console.log("Progression update rows:", r3.rowCount)
  ...
  const stats = await client.query(`
    SELECT
      ...
      COUNT(*) FILTER (WHERE academic_status = 'detained')::int AS detained,
      COUNT(*) FILTER (WHERE academic_status = 'year_back')::int AS year_back,
      ...
      COUNT(*) FILTER (WHERE needs_admission_year_review)::int AS needs_review,
